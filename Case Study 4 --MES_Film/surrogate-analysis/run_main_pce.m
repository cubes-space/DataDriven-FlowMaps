...
for jr=1:5
indices(jr,1)=train_starts(indt+jr); indices(jr,2)=train_ends(indt+jr);
end
close all
...
for l=2:npoints+1;
% flow map mode
delta = Xtraj(prev_ind+l-1,10);
tinit = tinit+delta;
time_arr = [time_arr, tinit];
% y1(l) = y1(l-1)+ (uq_evalModel(pce{1,1},[xinit,xs,delta]));
% y2(l) = y2(l-1)+ (uq_evalModel(pce{2,1},[xinit,xs,delta]));
% y3(l) = y3(l-1)+ (uq_evalModel(pce{3,1},[xinit,xs,delta]));


y1(l) = (uq_evalModel(pce{1,1},[xinit,xs,delta]));
y2(l) = (uq_evalModel(pce{2,1},[xinit,xs,delta]));
y3(l) = (uq_evalModel(pce{3,1},[xinit,xs,delta]));
