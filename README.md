...
## Usage
For each case study, you fill find a directory containing both Matlab and Python files. Matlab was used for modeling, simulation, and validation across a number of `.m` files. The results were saved as `.mat` files. Python was used for visualizing results in the form of a Jupyter notebook `.ipynb` file.
